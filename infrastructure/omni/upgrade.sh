...
AUTH0_DOMAIN="$(echo "$DECRYPTED_SECRET"        | yq -r '.stringData.AUTH0_DOMAIN')"
AUTH0_CLIENT_ID="$(echo "$DECRYPTED_SECRET"     | yq -r '.stringData.AUTH0_CLIENT_ID')"
AUTH0_CLIENT_SECRET="$(echo "$DECRYPTED_SECRET" | yq -r '.stringData.AUTH0_CLIENT_SECRET')"

for pair in "AUTH0_DOMAIN:$AUTH0_DOMAIN" \
            "AUTH0_CLIENT_ID:$AUTH0_CLIENT_ID" \
            "AUTH0_CLIENT_SECRET:$AUTH0_CLIENT_SECRET"; do
  name="${pair%%:*}"
  value="${pair#*:}"
  if [[ -z "$value" || "$value" == "null" || "$value" == REPLACE_ME_* ]]; then
